    ...
    public class User
    {
        ...
        public DataTable authenticateUser(string pUsername, string pPassword)
        {
            ...
            try
            {
                ...
                {
                    _conn.Open();
                    MySqlDataAdapter _da = new MySqlDataAdapter("exec spAuthenticateUser @Username=N'" + pUsername + "', @Password=N'"+pPassword+"'", _conn);
                    _da.Fill(_dt);
                    _conn.Close();
        ...
        public DataTable getUsers(string pDisplayType, string pPrimaryKey, string pSearchString)
        {
            ...
            {
                _conn.Open();
                MySqlDataAdapter _da = new MySqlDataAdapter("exec spGetUsers @DisplayType=N'" + pDisplayType + "', @PrimaryKey=N'" + pPrimaryKey + "', @SearchString=N'" + pSearchString + "'", _conn);
                _da.Fill(_dt);
                _conn.Close();
        ...
        public string insertUser(User pUser)
        {
            ...
            using (MySqlConnection _conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MySqlConnection"].ConnectionString))
            {
                _conn.Open();
                MySqlTransaction _trans = _conn.BeginTransaction();
                MySqlCommand _cmd = new MySqlCommand("exec spInsertUser @Username=N'" + pUser.Username +
                    "', @Password=N'" + pUser.Password +
                    "', @EmployeeId=N'" + pUser.EmployeeId +
                    "', @UserGroupId=N'" + pUser.UserGroupId + 
                    "', @Remarks=N'" + pUser.Remarks + 
                    "', @UsernameCreate=N'"+pUser.CreatedBy+"'", _conn);
                try
                {
                    _cmd.Transaction = _trans;
                    int _rowsAffected = _cmd.ExecuteNonQuery();
